...
<?php } else { ?>
	<?php if ( $product->get_lottery_closed() == 2 ) {?>
		<?php if ($product->is_user_participating()) : ?>
				<?php if(in_array($current_user->ID, $lottery_winers)): ?>
						<p><?php _e('Congratulations! You won!','wc_lottery') ?></p>
				<?php else: ?>
	...
	<?php } else{ 
		if ( $product->get_lottery_fail_reason() == '1' ) { ?>
			<p><?php _e('Competition failed because there were no participants','wc_lottery') ?></p>
		<?php } elseif ( $product->get_lottery_fail_reason() == '2' ) { ?>
			<p><?php _e('Competition failed because there was not enough participants','wc_lottery') ?></p>
		<?php } ?>
	<?php } ?>
