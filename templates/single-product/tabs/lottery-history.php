...
$heading = esc_html(  __( 'Competition History', 'wc_lottery' )  );
...
<?php if(($product->is_closed() === TRUE ) and ($product->is_started() === TRUE )) : ?>
	...
	<p><?php _e('Competition has finished', 'wc_lottery') ?></p>
	<?php if ($product->get_lottery_fail_reason() == '1'){
		 _e('Competition failed because there were no minimum users', 'wc_lottery');
	} else{

    	
    	if (count($lottery_winers) > 1) { ?>
           <p><?php _e('Competition winners are', 'wc_lottery') ?>: <?php foreach ($lottery_winers as $winner_id) {
        ...
        <?php } elseif(count($lottery_winers) == 1) { ?>
    		<p><?php _e('Competition winner is', 'wc_lottery') ?>: <span><?php echo get_userdata($lottery_winers[0])->display_name ?></span></p>
    	<?php } 
    } ?>
